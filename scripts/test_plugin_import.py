...
Run with Calibre's bundled Python so the real zip-plugin importer is used::
    ...
    calibre-debug scripts/test_plugin_import.py

A bare ``import sys.path`` shim (as in earlier versions of this script) gives a
false positive: it lets bare top-level imports such as ``hcl_graphql`` resolve
even though Calibre's importer only resolves names under ``calibre_plugins``.
So this script drives Calibre's own plugin machinery instead.
"""
...
assert DEFAULT_REQUESTS_PER_MINUTE, "hcl_graphql client incomplete"
